...
export default function CardModal({ type, card, onClose, onSave, onDelete }) {
  const isNew = !card.id;
        ...
        {type === "character" ? (
          <>
            ...
            <label className="field">
              ...
              <textarea rows={2} value={form.physical || ""} onChange={(e) => update("physical", e.target.value)} />
            </label>
            <label className="field">
          ...
          <>
            <label className="field">
              <span>Descripción física</span>
              <textarea rows={3} value={form.physical || ""} onChange={(e) => update("physical", e.target.value)} />
            </label>
            <label className="field">
              <span>Historia</span>
              <textarea rows={2} value={form.history || ""} onChange={(e) => update("history", e.target.value)} />
            </label>
            <label className="field">
              <span>Ambiente</span>
              <textarea rows={2} value={form.atmosphere || ""} onChange={(e) => update("atmosphere", e.target.value)} />
            </label>
          </>
        )}
